  ...
  private loadCommands(
    ...
    Object.keys(commands).forEach((command: string) => {
      ...
      if (currentRank > currentCommand.rank) {
        currentCommand.SetRank(currentRank);
        currentCommand.SetLifecycleEvents(commandInstance.lifecycleEvents);
      ...
      for (const option in commandInstance.options) {
        if (
          ...
          const { usage, config = {} } = commandInstance.options[option];
          currentCommand.option(option, usage, config);
        }
      }
